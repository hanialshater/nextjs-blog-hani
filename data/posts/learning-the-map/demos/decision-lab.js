;(function () {
  ...
  const content = {
    route: {
      number: '01',
      title: 'Good roads. Impossible route.',
      intro:
        'Six stops. One driver. A road can look cheap and still be the wrong choice for the whole journey.',
      instruction: 'Build a round trip',
      prompt:
        'Start at A. Click the other five stops in the order you would visit them. Your last stop connects back to A.',
      rule: 'Visit all six stops once, then return to A.',
      action: 'Drive this route',
      piece: 'road',
      pieces: 'roads',
      round: 'day',
      trap: 'Try the six cheapest roads',
      trapText:
        'Six cheap roads make two separate triangles. Every stop has two neighbours, but the driver cannot get from one triangle to the other. Local bargains do not guarantee a connected tour.',
      learn:
        'The driver returns with six travel times. The nine roads outside the route give you no new measurements.',
      compare: 'Does a better map lead to better journeys?',
      takeaway:
        'The solver keeps the tour connected. The learner decides which road costs deserve another measurement.',
    },
    matching: {
      number: '02',
      title: 'Everyone wants Mira.',
      intro:
        'Four clients. Four experts. Giving each client their favourite expert is easy—until two clients want the same person.',
      instruction: 'Give each client one expert',
      prompt:
        'Click a score in each client’s row. Each expert can take only one client. The numbers are starting estimates, not known outcomes.',
      rule: 'One expert per client. One client per expert.',
      action: 'Make these matches',
      piece: 'pair',
      pieces: 'pairs',
      round: 'round',
      trap: 'Give everyone their favourite',
      trapText:
        'Every client chose Mira. Those four attractive scores cannot be collected together: Mira has one appointment. A valid matching must consider what each choice leaves for everyone else.',
      learn:
        'Four matches return four outcomes. Pairings you did not make remain unobserved, even though every client met somebody.',
      compare: 'Four observations. But are they new?',
      takeaway:
        'A matching spreads observations across clients. It can still repeat exactly the same pairs forever. Exploration must come from the policy.',
    },
    schedule: {
      number: '03',
      title: 'One booking can teach you about another.',
      intro:
        'Four clients. Six appointments. Learn which bookings work, while respecting the calendar and using information about similar clients.',
      instruction: 'Make a feasible week',
      prompt:
        'Give each client one appointment. Hatched cells are unavailable for that client. Appointments in the same column cannot be shared.',
      rule: 'One appointment per client. No double bookings. Respect client availability.',
      action: 'Book this week',
      piece: 'booking',
      pieces: 'bookings',
      round: 'week',
      trap: 'Ignore client availability',
      trapText:
        'Bea was booked on Monday, but she cannot attend. Cam was booked with Noah, who cannot meet Cam’s required service. High estimated quality cannot make an unavailable booking feasible.',
      learn:
        'Observe four bookings. With shared learning, a result can also change the prediction for an untried booking with the same client type and expert specialty.',
      compare: 'When does sharing experience help?',
      takeaway:
        'Turn shared structure down and rerun the same seeds. Public labels are useful only when they help predict the hidden outcomes.',
    },
  }[kind]
  const names = {
    greedy: 'Trust the estimates',
    ucb: 'Explore uncertain pieces',
    thompson: 'Sample a plausible world',
    shared: 'Share experience + explore',
  }
  const colors = {
    greedy: 'var(--orange)',
    ucb: 'var(--blue)',
  ...
  let seed = 17,
    ...
    stage = 0,
    phase = 'choose',
    policy = kind === 'schedule' ? 'shared' : 'ucb'
  let world,
    state,
    picked,
    routeOrder,
  ...
  function reset() {
    ...
    busy = false
    comparison = null
    world = E.makeWorld(kind, seed, structure, constrained)
  ...
  function values() {
    return E.estimates(world, state, policy === 'shared')
  }
  function routeFromPlan(plan) {
    ...
    while (order.length < 6) {
      const edge = plan
        ...
        .find(
          (a) =>
            (a.a === current || a.b === current) &&
            (a.a === current ? a.b : a.a) !== previous &&
            !order.includes(a.a === current ? a.b : a.a)
        )
      if (!edge) break
  ...
  function makeRoute() {
    ...
    for (let i = 1; i < routeOrder.length; i++)
      picked.push(
        world.arms.findIndex(
          (a) =>
            a.a === Math.min(routeOrder[i - 1], routeOrder[i]) &&
            a.b === Math.max(routeOrder[i - 1], routeOrder[i])
        )
      )
    if (routeOrder.length === 6)
      picked.push(world.arms.findIndex((a) => a.a === 0 && a.b === routeOrder[5]))
  }
  function select(k) {
    inspected = k
    if (stage !== 0) {
    ...
    if (kind === 'route') {
      ...
      if (!a.allowed) {
        message = 'That appointment is unavailable. Choose an unhatched cell in this row.'
        render()
        return
      }
      const clash = picked.find((j) => world.arms[j].b === a.b && world.arms[j].a !== a.a)
      if (clash !== undefined) {
        message = `${world.cols[a.b]} is already booked for ${world.rows[world.arms[clash].a]}. Move that client first, or choose another column.`
        render()
        return
      }
  ...
  function showTrap() {
    trap = true
    message = content.trapText
    if (kind === 'route')
      picked = world.arms
        .map((a, k) => k)
        .sort((a, b) => world.arms[a].base - world.arms[b].base)
        .slice(0, 6)
  ...
  function fillPlan() {
    ...
    routeOrder = routeFromPlan(picked)
    trap = false
  ...
  function nextPhase() {
    if (phase === 'choose' || phase === 'updated') {
      proposal = E.choose(world, state, policy)
      picked = proposal.plan
      routeOrder = routeFromPlan(picked)
      phase = 'chosen'
      observations = []
      message = ''
      ...
      before = values()
      observations = E.observe(world, state.round, picked)
      phase = 'observed'
    } else {
      E.update(state, observations)
      phase = 'updated'
    }
    render()
  }
  ...
  function matrixBoard() {
    const est = values()
    return `<table class="matrix" aria-label="${kind === 'schedule' ? 'Client appointment' : 'Client expert'} estimates"><thead><tr><th scope="col">Client</th>${world.cols.map((c, i) => `<th scope="col">${c.replace(' · ', '<br>')}<small>${(kind === 'schedule' ? i % 2 === 0 : i < 2) ? 'Design' : 'Data'}</small></th>`).join('')}</tr></thead><tbody>${world.rows
      .map(
        (r, i) =>
          `<tr><th scope="row">${r}<small>${i < 2 ? 'Design' : 'Data'}</small></th>${world.cols
            .map((_, j) => {
              const k = i * world.cols.length + j,
                a = world.arms[k],
                selected = picked.includes(k),
                obs = observations.find((o) => o.k === k)
              const transferred =
                phase === 'updated' &&
                !selected &&
                before[k] &&
                Math.abs(est[k].mean - before[k].mean) > 0.01
  ...
  function learnView() {
    ...
    const transferred = updated
      ? values().filter(
          (e, k) =>
            world.arms[k].allowed !== false &&
            !picked.includes(k) &&
            before[k] &&
            Math.abs(e.mean - before[k].mean) > 0.01
        ).length
      : 0
    let explanation =
      phase === 'choose'
        ? 'Ask the policy to propose a valid plan. Then reveal the observations and update the model in separate steps.'
        : phase === 'chosen'
          ? 'The plan is committed. Before revealing feedback, look at the unselected pieces. None of them will receive a new measurement.'
          : observed
  ...
  async function runComparison() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      document.getElementById('progress').textContent =
        `Evaluated ${i + 1} of ${count} ${count === 1 ? 'world' : 'worlds'}…`
      await new Promise((resolve) => setTimeout(resolve, 0))
    }
    if (token !== raceToken) return
    comparison = {
      count,
      results: sets[0].map((r, p) => ({
        policy: r.policy,
        coverage: sets.reduce((s, a) => s + a[p].coverage, 0) / count,
        history: r.history.map((_, t) => ({
          gap: sets.reduce((s, a) => s + a[p].history[t].gap, 0) / count,
          regret: sets.reduce((s, a) => s + a[p].history[t].regret, 0) / count,
        })),
  ...
  function chartView() {
    ...
    const max =
      Math.max(1, ...comparison.results.flatMap((r) => r.history.map((h) => h[metric]))) * 1.1
    const x = (t) => 48 + t * 6.9,
      y = (v) => 218 - (v / max) * 186
  ...
  function render() {
    ...
    const focusKey =
      active && active !== document.body
        ? ['data-action', 'data-arm', 'data-inspect', 'data-node', 'data-metric']
            .map((a) => (active.hasAttribute(a) ? `[${a}="${active.getAttribute(a)}"]` : ''))
  ...
  app.addEventListener('click', (event) => {
    ...
    if (el.dataset.inspect !== undefined) {
      inspected = Number(el.dataset.inspect)
      render()
      return
    }
