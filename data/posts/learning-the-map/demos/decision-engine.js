...
;(function (root) {
  ...
  function makeWorld(kind, seed = 17, structure = 1, constrained = true) {
    ...
    const cols =
      kind === 'schedule'
        ? ['Mira · Mon', 'Noah · Mon', 'Mira · Tue', 'Noah · Tue', 'Mira · Wed', 'Noah · Wed']
        : ['Mira', 'Noah', 'Omar', 'Rae']
    if (kind === 'route') {
      for (let a = 0; a < 6; a++)
        for (let b = a + 1; b < 6; b++) {
      ...
      function visit(row, selected, used) {
        ...
        arms.forEach((arm, k) => {
          if (arm.a === row && arm.allowed && !used.includes(arm.b))
            visit(row + 1, [...selected, k], [...used, arm.b])
        })
      }
      visit(0, [], [])
    }
    const groupEffects = Array.from(
      { length: kind === 'route' ? 3 : 4 },
      () => (rng() - 0.5) * (kind === 'route' ? 13 : 30)
    )
    const truth = arms.map((arm) => {
      const privateEffect = (rng() - 0.5) * (kind === 'route' ? 8 : 24)
      return (
        arm.base + structure * groupEffects[arm.group] + (1.35 - 0.6 * structure) * privateEffect
      )
    })
    const world = {
      kind,
      ...
      cols,
      minimize: kind === 'route',
      noise: kind === 'route' ? 3 : 8,
      unit: kind === 'route' ? 'min' : 'points',
    }
  ...
  function solve(world, values) {
    let best = world.plans[0],
      bestValue = sum(best, values)
    for (const plan of world.plans) {
      const v = sum(plan, values)
      if (world.minimize ? v < bestValue : v > bestValue) {
        best = plan
        bestValue = v
      }
    }
  ...
  function newState(world) {
    return { n: world.arms.map(() => 0), total: world.arms.map(() => 0), round: 0 }
  }
  ...
  function gap(world, plan) {
    return Math.max(
      0,
      world.minimize
        ? sum(plan, world.truth) - world.optimalValue
        : world.optimalValue - sum(plan, world.truth)
    )
  }
  ...
  function race(world, rounds = 100, exploration = 1) {
    return ['greedy', 'ucb', 'thompson', 'shared'].map((policy) => {
      ...
      for (let t = 0; t < rounds; t++) {
        ...
        const recommendation = solve(
          world,
          estimates(world, state, policy === 'shared').map((e) => e.mean)
        )
      ...
      return {
        policy,
        history,
        coverage: state.n.filter((n, k) => n > 0 && world.arms[k].allowed !== false).length,
      }
    })
  }
  const api = {
