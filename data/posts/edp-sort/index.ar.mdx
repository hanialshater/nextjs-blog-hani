...
summary: 'الترتيب التقليدي بفترض مقارنة موثوقة. الحياة بتعطيك إشارات ناقصة وفيها ضجيج. من اختيار مطعم، لترتيب لاعبين من مباريات غير مضمونة، لاختيار top-K كاملة: الفكرة وحدة — وين تصرف محاولتك الجاية؟'
authors: ['default']
layout: 'PostLayout'
...
2. **قناة التغذية الراجعة** — شو العالم بيرجعلك بعد ما تختار.
...
إنت ما بتاكل بكل المطاعم بنفس الليلة. ما بتوصلك نتائج كل المطاعم. بتختار مطعم واحد، بتشوف نتيجة واحدة فيها ضجيج، والباقي بضل احتمالات.
...
المعادلة دفتر حساب بسيط. بكل ليلة، قارن المطعم اللي اخترته، $\mu_{a_t}$، مع أفضل معدل مخفي، $\mu^*$. اجمع الفرق.
...
سياسة الـ bandit هي القاعدة اللي بتختار الـ pull الجاية. عملياً، هي طريقة لصرف ميزانية التعلّم: أي عدم يقين لسه بستاهل تدفع ثمنه؟
...
KL هون هي مسافة إحصائية بين معدل Bernoulli اللي شفته، $\hat{\mu}_i$، والمعدل المرشح $q$.
...
**Thompson sampling** — _خلّي المعتقدات تتسابق._ احتفظ بتوزيع احتمالي لكل arm، مش متوسط واحد. بكل جولة، اسحب جودة محتملة من كل توزيع وروح على الأعلى.
...
هون الـ context بيفيد. عشا عيلة ناجح مش دليل عن هاي الليلة فقط؛ هو بيعلّم الموديل عن مواقف عائلية ثانية. غدا شغل سريع بيعلّم عن سياقات quick-work ثانية. النموذج ببطل يخلط مواقف ما لازم تنخلط.
...
لما نفس المطعم يكون جيد بسياق وسيء بسياق ثاني، السياسة السياقية متوقع تراكم خسارة أقل لأنها بتوقف تجمع عشاوات غير متوافقة في معدل واحد.
...
قوة النموذج إنه بنقل أثر المعلومة بين المقارنات. إذا A غلب B، وB غلب C، الـ scale المشترك صار يحكي شيء عن A مقابل C قبل ما يلعبوا.
...
<Demo src="/demos/posts/edp-sort/pairing-race.ar.html" title="وين تصرف مبارياتك" height={440} />

المنحنى هو متوسط **استرجاع أفضل خمسة (top-5 recall)** عبر بطولات محاكاة متكررة: قديش من أفضل خمسة حقيقيين دخلوا قائمتنا المقدّرة؟ المقياس بين صفر وواحد. المنطقة المبكرة أهم، لأن ميزانية مباريات كبيرة بتعطي سياسات كثيرة وقتاً تلحق.
...
لو $K=100$، فالـ cutline هو الحد الحالي حول الرتبة 100: المكان اللي لسه مش محسوم مين يدخل المجموعة ومين يطلع منها.
...
بهالإعداد المصطنع للمحاكاة، random pairing بيحرق مباريات بعيد عن cutline. Adaptive matching بيرجع معظم top-100 بعشرات الآلاف من المباريات بدل ما يعدّ خمسين مليون زوج.
...
الجزء الثاني خفّض الفاتورة بإنه صرف المقارنات حول cutline. الجزء الثالث بستخدم طريقة ثانية: اختار slate، شوف feedback على العناصر المختارة، وحدّث هدول بس.
...
هنا $\hat{r}_i$ هو reward estimate الحالي، و$n_i$ عدد مرات وصول feedback قابل للاستخدام لـ item رقم $i$.
...
هلأ خذ كومة حقيقية: آلاف مراجعات الزبائن، وبدك تطلع منها كم جملة بتستاهل القراءة.
...
| pairwise constraints | — | غير محسوم؛ شوف الملاحظة تحت |
