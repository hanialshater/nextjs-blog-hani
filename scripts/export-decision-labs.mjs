// Package the exact blog demos into one shareable review file; Berlin map tiles require internet.
...
const demos = Object.fromEntries(
  [
    ...
    kind,
    read(`${file}-semibandit.html`)
      .replace(
        '<link rel="stylesheet" href="semibandit-guide.css" />',
        () => `<style>${read('semibandit-guide.css')}</style>`
      )
      .replace(
        '<script src="semibandit-guide.js"></script>',
        () => `<script>${read('semibandit-guide.js')}</script>`
      )
      .replace(
        'href="solver-models.py"',
        () =>
          `href="data:text/x-python;base64,${Buffer.from(read('solver-models.py')).toString('base64')}"`
      ),
  ])
)
...
</style></head><body><nav class="bar" aria-label="Choose an experiment"><strong>Decision laboratory</strong><button data-kind="route" aria-pressed="true">Berlin52</button><button data-kind="matching" aria-pressed="false">Matching</button><button data-kind="schedule" aria-pressed="false">Scheduling</button><label>Reading width <select id="width"><option value="wide">Full</option><option value="narrow">Phone</option></select></label></nav><iframe id="demo" title="Routing experiment" sandbox="allow-scripts allow-downloads" height="900"></iframe><script>
