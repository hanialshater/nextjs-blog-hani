...
async function expectCanonical(page, expectedHref: string) {
  const canonical = page.locator('link[rel="canonical"]')
  await expect(canonical).toHaveCount(1)
  await expect(canonical).toHaveAttribute('href', expectedHref)
}

async function expectLanguageAlternates(page, path: string) {
...
test.describe('SEO metadata', () => {
  ...
  test('sitemap.xml exposes localized index pages and does not expose missing tags pages', async ({
    ...
    const body = await response.text()
    for (const path of ['', '/blog', '/free-writing', '/projects', '/about']) {
