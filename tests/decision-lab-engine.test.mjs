...
import E from '../data/posts/learning-the-map/demos/decision-engine.js'
  ...
  for (const kind of ['route', 'matching', 'schedule']) {
    ...
    for (const p of w.plans) {
      ...
      if (kind === 'route') {
        ...
        for (let i = 0; i < 6; i++)
          arms.forEach((a) => {
            if (reached.has(a.a) || reached.has(a.b)) {
              reached.add(a.a)
              reached.add(a.b)
            }
          })
        assert.deepEqual(degrees, [2, 2, 2, 2, 2, 2])
...
test('the three tempting shortcuts are genuinely invalid', () => {
  ...
  const cheapest = r.arms
    .map((_, k) => k)
    .sort((a, b) => r.arms[a].base - r.arms[b].base)
    .slice(0, 6)
...
test('calendar ablation changes only feasible plans, preserving all potential outcomes', () => {
  const on = E.makeWorld('schedule', 21, 1, true),
    off = E.makeWorld('schedule', 21, 1, false)
...
test('feedback updates selected pieces only; shared predictions can move without new direct samples', () => {
  const w = E.makeWorld('schedule'),
    s = E.newState(w)
  const k = 0,
    peer = w.arms.findIndex((a, j) => j !== k && a.group === w.arms[k].group && a.allowed)
  const before = E.estimates(w, s, true)
  E.update(s, [{ k, value: w.arms[k].base + 20 }])
  const after = E.estimates(w, s, true),
    tabular = E.estimates(w, s)
  assert.equal(s.n[k], 1)
  assert.equal(s.n[peer], 0)
...
test('all policies choose valid plans without consulting hidden truth or evaluation optimum', () => {
  for (const kind of ['route', 'matching', 'schedule'])
    for (const policy of ['greedy', 'ucb', 'thompson', 'shared']) {
      const w = E.makeWorld(kind),
        s = E.newState(w)
      Object.defineProperty(w, 'truth', {
        get() {
          throw Error('hidden truth accessed')
        },
      })
      Object.defineProperty(w, 'optimalValue', {
        get() {
          throw Error('oracle accessed')
        },
      })
      for (let t = 0; t < 8; t++) {
        ...
        E.update(
          s,
          p.plan.map((k) => ({ k, value: w.arms[k].base + (t % 3) }))
        )
      }
    }
})
...
test('outcomes depend on world, time and piece, not policy, plan order or policy execution order', () => {
  ...
  assert.deepEqual(
    E.observe(w, 7, p)
      .slice()
      .sort((a, b) => a.k - b.k),
    E.observe(w, 7, q).sort((a, b) => a.k - b.k)
  )
...
test('reported recommendation gap evaluates a mean-selected plan, and cumulative regret accounts for executed actions', () => {
  const w = E.makeWorld('schedule', 17),
    result = E.race(w, 12).find((r) => r.policy === 'shared')
  const s = E.newState(w)
  let bill = 0
  for (let t = 0; t < 12; t++) {
    ...
    const recommend = E.solve(
      w,
      E.estimates(w, s, true).map((e) => e.mean)
    )
