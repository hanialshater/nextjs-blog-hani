...
export default function MapElitesDemo() {
    ...
    const isValid = (c: Circle[]) => {
        for (let i = 0; i < N; i++) {
            const ci = c[i]
            if (ci.x - ci.r < -1e-9 || ci.x + ci.r > 1 + 1e-9 ||
                ci.y - ci.r < -1e-9 || ci.y + ci.r > 1 + 1e-9) return false
        }
        for (let i = 0; i < N; i++) {
            for (let j = i + 1; j < N; j++) {
    ...
    const applyVF = (c: Circle[]) => {
        ...
        for (let iter = 0; iter < 500; iter++) {
            ...
            for (let i = 0; i < N; i++) {
                const ci = c[i]
                // Boundaries
                if (ci.x < ci.r) { fx[i] += ci.r - ci.x; maxF = Math.max(maxF, ci.r - ci.x) }
                if (ci.x > 1 - ci.r) { fx[i] -= ci.x - 1 + ci.r; maxF = Math.max(maxF, ci.x - 1 + ci.r) }
                if (ci.y < ci.r) { fy[i] += ci.r - ci.y; maxF = Math.max(maxF, ci.r - ci.y) }
                if (ci.y > 1 - ci.r) { fy[i] -= ci.y - 1 + ci.r; maxF = Math.max(maxF, ci.y - 1 + ci.r) }
    ...
    const crossover = (p1: Circle[], p2: Circle[], numSwaps: number) => {
        ...
        for (let m = 0; m < numSwaps; m++) {
            ...
            for (let j = 0; j < N; j++) {
                const dx = p1[i].x - p2[j].x, dy = p1[i].y - p2[j].y
                const d = dx * dx + dy * dy
    ...
    const addToArchive = (c: Circle[], targetCell: Cell | null = null) => {
        const f = fitness(c)
    ...
    const getBestFromCell = (x: number, y: number) => {
    ...
    const init = useCallback(() => {
        archiveRef.current = Array(GRID).fill(0).map(() => Array(GRID).fill(0).map(() => []))
        ...
        for (let i = cellQueueRef.current.length - 1; i > 0; i--) {
            const j = (Math.random() * (i + 1)) | 0;
            [cellQueueRef.current[i], cellQueueRef.current[j]] = [cellQueueRef.current[j], cellQueueRef.current[i]]
        }
        ...
        // Initial UI update
        setStats({
            bestR: 0, steps: 0, archiveSize: 0, filledCells: 0,
            phase: 'SEED', phaseIter: 0, phaseMax: SEED_ITERS, valid: false
        })
        draw()
    }, [])
    ...
    const draw = () => {
        ...
        const circlesToDraw = (viewingCellRef.current && archiveRef.current[viewingCellRef.current.x][viewingCellRef.current.y].length > 0)
    ...
    const animate = useCallback(() => {
        ...
        if (stepsRef.current % 200 === 0) {
            let filled = 0
            for (let x = 0; x < GRID; x++) for (let y = 0; y < GRID; y++) if (archiveRef.current[x][y].length > 0) filled++
            ...
            setStats({
                ...
                phaseMax: focusPhaseRef.current === 'seed' ? SEED_ITERS :
                    focusPhaseRef.current === 'hc' ? HC_ITERS :
                        focusPhaseRef.current === 'es' ? ES_ITERS : 0,
    ...
    const manualStep = (n: number) => {
        if (!archiveRef.current.length) return
    ...
    return (
        <div className="my-8 rounded-xl border border-gray-700 bg-gray-900 p-4 shadow-2xl">
            <div className="flex flex-col gap-6 md:flex-row">
                ...
                <div className="relative aspect-square w-full md:w-[400px]">
                    ...
                    <div className="absolute top-2 left-2 text-xs font-mono text-cyan-400">
                        {viewingCellRef.current ? `VIEW: Cell (${viewingCellRef.current.x}, ${viewingCellRef.current.y})` : 'VIEW: Global Best'}
                    </div>
                </div>
                ...
                <div className="flex flex-1 flex-col gap-4 font-mono text-sm text-gray-300">
                    <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={() => manualStep(100)} className="rounded bg-gray-800 px-2 py-1 hover:bg-gray-700">+100</button>
                        <button onClick={() => manualStep(1000)} className="rounded bg-gray-800 px-2 py-1 hover:bg-gray-700">+1k</button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div className="bg-gray-800 p-2 rounded border border-gray-700">
                            ...
                            <div className="text-xl font-bold">{stats.filledCells}<span className="text-gray-600 text-sm">/100</span></div>
                        </div>
                        <div className="col-span-2 bg-gray-800 p-2 rounded border border-gray-700 flex justify-between">
                            ...
                            <div className="text-right">
                                ...
                                <div className={`font-bold ${stats.phase === 'DONE' ? 'text-green-400' : 'text-yellow-400'}`}>
                                    {stats.phase} {stats.phase !== 'DONE' && stats.phase !== '-' && `(${stats.phaseIter}/${stats.phaseMax})`}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex justify-between items-end">
                            <span className="text-xs text-gray-400">Archive Grid <span className="text-gray-600">(Click cells to inspect)</span></span>
                            <button onClick={handleGlobalClick} className="text-xs text-cyan-400 hover:underline">View Global</button>
                        </div>
                        <div
                            ...
                            {(() => {
                                ...
                                for (let y = GRID - 1; y >= 0; y--) {
                                    for (let x = 0; x < GRID; x++) {
                                        ...
                                        const isViewing = viewingCellRef.current?.x === x && viewingCellRef.current?.y === y
                                        // Check if focus cell
            ...
            <Caption>
                Interactive MAP-Elites simulation. The grid represents the behavioral space (Symmetry vs. Radius Variance). The algorithm illuminates the map by finding the best circle packing for each cell. <strong>Click on any filled grid cell to view that specific solution.</strong>
            </Caption>
        </div>
    )
}
