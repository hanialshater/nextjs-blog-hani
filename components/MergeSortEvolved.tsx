...
const MergeSortEvolved = () => {
  ...
  const buildTreeRecursive = useCallback(
    (arr: number[], start: number, end: number, depth: number, index: number): TreeNode | null => {
  ...
  return (
    <div
      className="mx-auto max-w-5xl rounded-xl bg-white p-5 shadow-lg dark:bg-slate-900"
      style={{ height: '600px' }}
    >
      <div className="mb-4">
        <h3 className="mb-1 text-xl font-bold text-slate-800 dark:text-slate-100">
          Merge Sort - Recursive Tree Visualization
        </h3>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          onClick={handlePause}
          disabled={!isPlaying}
          className="rounded-lg bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-300 disabled:opacity-50 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
        >
          Pause
        </button>
        <button
        ...
        <button
          onClick={handleReset}
          className="rounded-lg bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
        >
          Reset
        </button>
        ...
        <div className="ml-auto flex gap-5 text-sm text-slate-600 dark:text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-indigo-500">{comparisons}</span>
          </div>
          <div className="flex items-center gap-2">
            <span>Merges:</span>
            <span className="text-lg font-bold text-indigo-500">{merges}</span>
          </div>
          <div className="flex items-center gap-2">
            <span>Phase:</span>
            <span className="text-lg font-bold text-indigo-500">{phase}</span>
          </div>
        </div>
      </div>

      <div className="mb-4 flex min-h-[44px] items-center rounded-lg bg-slate-100 px-4 py-3 text-sm text-slate-700 dark:bg-slate-800 dark:text-slate-300">
        {statusMessage}
      </div>

      <div
        ref={containerRef}
        className="relative flex-1 rounded-lg border-2 border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800"
        style={{ height: '350px' }}
      >
        <canvas ref={canvasRef} className="h-full w-full" />
        <div className="absolute top-4 right-4 rounded-lg bg-white p-3 text-xs shadow-md dark:bg-slate-900">
          <div className="mb-2 flex items-center gap-2">
            <div className="h-3 w-5 rounded bg-blue-500"></div>
            <span className="text-slate-600 dark:text-slate-400">Dividing</span>
          </div>
          <div className="mb-2 flex items-center gap-2">
            <div className="h-3 w-5 rounded bg-emerald-500"></div>
            <span className="text-slate-600 dark:text-slate-400">Merging</span>
          </div>
          <div className="mb-2 flex items-center gap-2">
            <div className="h-3 w-5 rounded bg-amber-400"></div>
            <span className="text-slate-600 dark:text-slate-400">Sorted</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-3 w-5 rounded bg-red-500"></div>
